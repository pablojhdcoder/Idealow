...
function aggregateScore(results) {
    ...
    const validation_score = totalWeight > 0 ? Math.round(weightedSum / totalWeight) : 0;
    return {
        validation_score,
        verdict: getVerdict(validation_score),
        recommendation: getRecommendation(validation_score),
        breakdown: Object.fromEntries(Object.entries(WEIGHTS).map(([source, weight]) => {
            const s = results[source]?.score;
...
function getRecommendation(score) {
    if (score >= 75)
        return 'Strong market signals. Build the MVP now.';
    if (score >= 55)
        return 'Moderate signals. Validate further before building.';
    if (score >= 35)
        return 'Weak signals. Refine the problem or pivot the solution.';
